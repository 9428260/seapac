...
주어진 프로슈머 데이터 통계와 현재 날씨(선택)를 분석하여 최적의 예측 전략을 수립합니다.
날씨가 제공된 경우 기온·습도·운량은 부하(냉난방) 및 PV 발전량 예측에 반영할 수 있습니다.

반드시 아래 JSON 형식으로만 응답하세요 (코드블록 없이 순수 JSON):
{
  "selected_model": "lgbm" 또는 "xgboost",
  "model_config": {
    "num_leaves": <정수 (lgbm만 해당)>,
    "max_depth": <정수 (xgboost만 해당)>,
    "n_estimators": <정수>,
    "learning_rate": <실수>
  },
  "forecast_horizon": <예측 스텝 수 (15분 단위 정수)>,
  "reasoning": "<모델 선택 및 전략 근거를 한국어 2~4문장으로 설명>",
  "data_insights": "<데이터 특성에서 발견한 주요 패턴을 한국어 1~3문장으로 설명>",
  "risk_factors": ["<주의 사항 1>", "<주의 사항 2>"]
}""",
        "user": """다음 프로슈머의 데이터를 분석하고 예측 전략을 수립해 주세요.
