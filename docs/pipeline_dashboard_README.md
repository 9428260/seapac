...
- **새 파이프라인 실행**: 상단 폼에서 데이터(`test_2026may_seoul.pkl` 등) 선택, 프로슈머·스텝(1일=96)·Parallel Agents 옵션 설정 후 **전체 아키텍처 실행** 클릭 → 해당 Run 상세 페이지로 이동하며 단계별 결과가 순차 표시됨. 실행 중에는 3초마다 자동 새로고침.
